...
class EnvironmentTeardown:
    ...
    def execute(self):
        ...
        # Try to delete portchannel on the service, if it exists
        self._execute_delete_portchannel(api, reservation_details)
        self.logger.info('trying to delete portchannels, if they exists')
    ...
    def _execute_delete_portchannel(self, api, reservation_details):
        services = reservation_details.ReservationDescription.Services
        for service in services:
            if service.ServiceName == 'Port Channel Service':
                api.ExecuteCommand(self.reservation_id, service.Alias, 'Service', 'delete_port_channel')
    ...
    def _clean_teraVMTest(self, api):
        resources = api.GetReservationDetails(self.reservation_id).ReservationDescription.Resources
        for res in resources:
            if res.ResourceFamilyName == 'TVM_Test':
                try:
                    api.ExecuteCommand(reservationId=self.reservation_id,
                                       targetName=res.Name,
                                       targetType='Resource',
                                       commandName='rearrange_gateways',
                                       commandInputs=[api.InputNameValue('direction', 'back')]
                                       )
                except:
                    api.WriteMessageToReservationOutput(reservationId=self.reservation_id,
                                                        message='can\'t rearrange')
                try:
                    api.ExecuteCommand(reservationId=self.reservation_id,
                                       targetName=res.Name,
                                       targetType='Resource',
                                       commandName='Stop_Test',
                                       )
                except:
                    api.WriteMessageToReservationOutput(reservationId=self.reservation_id,
                                                        message='can\'t stop test')
                # try:
                #     api.ExecuteCommand(reservationId=self.reservation_id,
                #                    targetName=res.Name,
                #                    targetType='Resource',
            ...
            api.SetResourceLiveStatus(res.Name, '')