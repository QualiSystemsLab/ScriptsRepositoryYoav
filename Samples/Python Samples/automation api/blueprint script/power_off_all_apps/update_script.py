...
FLOW: - DEBUG_PRINT set to False
      ...
      - updated on cloud-shell
NOTE: - This script is only for updating EXISTING scripts.
      - Upload manually first time to portal. (Script can still be used to zip)
"""
...
# To name script something other than directory name
CUSTOM_SCRIPT_NAME = ''
...
def switch_off_debug_print():
    ...
    debug_file_name = "DEBUG.py"
    debug_file_path = os.getcwd() + "\\" + debug_file_name
    debug_file_exists = os.path.isfile(debug_file_path)
    if debug_file_exists:
        try:
            inplace_change(filename=debug_file_path,
                           curr_pattern="DEBUG_PRINT = True",
                           new_string="DEBUG_PRINT = False")
        except Exception as e:
            print(error_red("[-] Issue updating {debug_file}\n".format(debug_file=debug_file_path)
                            + str(e)))
        else:
...
def zip_files():
    ...
    files_to_exclude = [zip_file_name, "credentials.py", "venv", ".idea"]
