...
class MynewfirewallDriver (ResourceDriverInterface):
    ...
    def get_inventory(self, context):
        ...
        resource = Mynewfirewall.create_from_context(context)

        try:
            resource.vendor = context.resource.address
        except:
            pass
        ...
        for i in range(1, int(resource.ports)):
            port1 = GenericPort('Port {}'.format(str(i)))
    ...
    def some_command(self, context, cancellation_context, some_input="default"):
        """
        :param ResourceCommandContext context: The context object for the command with resource and reservation info
        :param CancellationContext cancellation_context: Object to signal a request for cancellation. Must be enabled in drivermetadata.xml as well
        :return: the command result text
