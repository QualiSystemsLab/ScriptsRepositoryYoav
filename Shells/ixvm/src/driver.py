...
from license_ssh_handler import configure_license
from cloudshell.api.cloudshell_api import CloudShellAPISession

class IxvmDriver (ResourceDriverInterface):
    ...
    def get_inventory(self, context):
        ...
        resource = Ixvm.create_from_context(context)
        resource.vendor = 'Ixia'
        resource.model = 'IXVM'


        # license_handler = configure_license(
        #     hostname=context.resource.address,
        #     user=resource.user,
        #     password=session.DecryptPassword(resource.password).Value,
        #     lic_server_address=resource.license_server,
        #     port=22
        # )
        # license_handler.config_license()
    ...
    def reconfigure_license(self, context):
        '''
        ...
        session.WriteMessageToReservationOutput(res_id, context.resource.address)
        session.WriteMessageToReservationOutput(res_id, resource.user)
        session.WriteMessageToReservationOutput(res_id, session.DecryptPassword(resource.password).Value)
